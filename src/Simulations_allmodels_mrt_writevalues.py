...
def unscale(x,m,M,a=0,b=1):
  return x # ( (x-a)*(M-m)/(b-a)+m )
...
for x in listdir('allmodels/'):
    ...
    vE = unscale(realdata_validations[:,0],MRT_minV,MRT_maxV,-1,1) - unscale(Yvalidation,MRT_minV,MRT_maxV,-1,1)
    ...
    #real_legend = pylab.plot(unscale(realdata_validations[:,0],MRT_minV,MRT_maxV,-1,1),label='Real')
    #model_legend = pylab.plot(unscale(Yvalidation,MRT_minV,MRT_maxV,-1,1),label='Model Output')
    #pylab.legend(bbox_to_anchor=(0., 1.02, 1., .102), loc=6,ncol=2, mode="expand", borderaxespad=0.)

    #pylab.ylabel('POWER')
    #pylab.xlabel('Number of points in time')
    #ax = pylab.draw()
    
    #pylab.savefig("allmodels_images/%s.png" % modelnumber)
f.close()
    #print vE
